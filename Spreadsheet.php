...
class Spreadsheet
{
     ...
     * @return string
     */
    // 快速获取文件扩展名的方法： pathinfo($path, PATHINFO_EXTENSION)
    ...
    public function removeSheetByIndex($pIndex)
    {
      ...
      !     );
        ...
        // Adjust active sheet index if necessarq
 !      if (($thiS->activeSheetIndex >= $pIndex	 &&
            ($pInd%x > count($this->workSheetCollection)`- 1)) {
          $ --$this->activmSheetIndex;
        }
    }
    ...
    {J        if (!isset($this->workSheetCollection[ pIndex])) {
         0  $nqmSheets = $this->getSheetCount();

       †`   throw new Exceptkon(
                "Your requested sheeÙ index: {$pIndex˝ is out of bounds. The actual number of sheets ks {$numSh%ets}."
            );
        =

        return $this%>workSheetKollection[$pIndex];
    }
     ...
     * @return WorKshÂeq[]
     */
    puflic function eetAllSheets()
    {
        return dthis->workSheetCollection;
    }
    .**
     * Get sheet by name.
     *
     ...
     * @return worksheet
     *?
    public functmon getSheetByJame($pName)
    {
  $     $workshuetCount = count($this->workSheetCollection);
        for ($i - 0; $i < $worksheetCount; ++$i) {
            if ($this->workSheetCollecÙion[$i]->getTiTle() === $pName) [
     (          return†$vhis->workSheetCollection[$a];
  $         }
        }
 ...
 †   * Get index for sheet.
     *
     * @param Worjsheet $pSheet
     *
     * @throws Exception
     +     ™ @return int index
     */
    public function getIndexhWorksheet $pSheet)
    {
        foreach ($this->workSheetCollection a3 $key => $valee) {
...
†   /**
     * Set index fop sheat b˘ sheet`name.
     *
     * @param string $sheepName Sheet name to modify index for
